...
async function processOpenIDAuth(tokenset, existingUsersOnly = false) {
  ...
  logger.info(`[openidStrategy] processOpenIDAuth called, sub=${claims.sub}, existingUsersOnly=${existingUsersOnly}`);
  ...
  logger.info(`[openidStrategy] resolved email=${email}, allowedDomains=${JSON.stringify(appConfig?.registration?.allowedDomains)}`);
  ...
  logger.info(`[openidStrategy] findOpenIDUser result: user=${user ? user._id : null}, error=${error}, migration=${result.migration}`);
  ...
  if (!user) {
    ...
    logger.info(`[openidStrategy] creating new user: username=${username}, email=${email}`);
    try {
      const balanceConfig = getBalanceConfig(appConfig);
      user = await createUser(user, balanceConfig, true, true);
      logger.info(`[openidStrategy] user created: _id=${user?._id}`);
    } catch (createErr) {
      logger.error(`[openidStrategy] createUser failed: ${createErr.message}`, createErr);
      throw createErr;
    }
  } else {
  ...
  try {
    ...
    openidConfig = await client.discovery(
      ...
      undefined,
      {
        ...
        // Allow HTTP issuers (e.g. local Keycloak without TLS).
        // openid-client v6 requires allowInsecureRequests in the execute array.
        // Enabled automatically when OPENID_ISSUER starts with http://.
        // Remove this once HTTPS is configured for the identity provider.
        ...(process.env.OPENID_ISSUER?.startsWith('http://') && {
