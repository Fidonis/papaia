...
# that must stay in sync with the service .env files):
#
...
# Files that are ALWAYS overwritten on sync, even without --force.
# These carry the realm's client secrets and must stay in lockstep
# with the secrets baked into the service .env files. Keeping a stale
# copy silently breaks OIDC token exchanges with invalid_client_credentials.
ALWAYS_OVERWRITE=(
    "infra/keycloak/realm-import/papaia-realm.json"
)
...
# Is the given relative path in ALWAYS_OVERWRITE?
is_always_overwrite() {
    local rel="$1"
    local entry
    for entry in "${ALWAYS_OVERWRITE[@]}"; do
        if [ "$rel" = "$entry" ]; then
            return 0
        fi
    done
...
copy_one() {
    ...
    if [ -e "$dst" ] && [ "$FORCE" -ne 1 ] && [ "$forced" -ne 1 ]; then
...
for f in "${ALWAYS_OVERWRITE[@]}"; do copy_one "$f"; done
